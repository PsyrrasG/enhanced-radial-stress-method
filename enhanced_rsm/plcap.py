...
The numerical kernels are imported from :mod:`enhanced_rsm.core`; the Tee
...
from :mod:`enhanced_rsm.el1` unless supplied explicitly.
"""
...
class PLCAPResult(NamedTuple):
    ...
    theta_LMS, pl_ratio_LMS, n_LMS : int, float, float
        Critical plane on the lower moment side (same with/without
        redistribution).
    theta_HMS, pl_ratio_HMS, n_HMS : int or None, float or None, float or None
        Higher moment side state at the plastic capacity, before redistribution.
    theta_HMS_RE, pl_ratio_HMS_RE, n_HMS_RE : int or None, float or None, float or None
    ...
    pl_ratio_LMS: float
    n_LMS: float
    theta_HMS: Optional[int]
    pl_ratio_HMS: Optional[float]
    n_HMS: Optional[float]
    theta_HMS_RE: Optional[int]
...
def _make_initial_state():
    ...
    return {
        "critical_theta_found": False,
        "critical_theta_completed": False,
...
def _perform_enhanced_rsm(sector, N_Q, Ved, N_T, props, n_expanded, n_values,
    ...
    -------
    tuple
        ``(M_Rd_tot, M_Ed, r, critical_theta_info, plasticity_level_max_info,
        critical_theta_completed, pl_ratio_values)``.
    """
    ...
    pl_ratio_fully_yielded = pl_ratio_values[THETAS, n_idxs_fully_yielded]

    # Branch 1 — capture the LMS critical plane (first full yield), once.
    if np.any(fully_yielded_mask) and not critical_theta_found:
        critical_theta_idx = int(np.argmax(pl_ratio_fully_yielded))
        critical_theta_info = {
            "sector": sector,
            "theta": critical_theta_idx,
            "pl_ratio": pl_ratio_fully_yielded[critical_theta_idx],
            "n": n_values[n_idxs_fully_yielded[critical_theta_idx]],
        }
    else:
    ...
    if critical_theta_found and not critical_theta_completed:
        ...
        critical_theta_completed = True
    else:
        plasticity_level_max_info = None

    return (M_Rd_tot, M_Ed, r, critical_theta_info,
            plasticity_level_max_info, critical_theta_completed, pl_ratio_values)
...
def _compute_tee_max_capacity(LMS_sector, HMS_sector, state, Ved, N_T, N_Q_map,
    ...
    """
    # --- Lower moment side ---
    (LMS_M_Rd_tot, LMS_M_Ed, _, LMS_critical_theta_info, _,
     state["critical_theta_completed"], _) = _perform_enhanced_rsm(
        LMS_sector, N_Q_map[LMS_sector], Ved, N_T, props, n_expanded, n_values,
        state["critical_theta_found"], state["critical_theta_completed"],
    )

    if LMS_critical_theta_info:
        state["critical_theta_found"] = True
        state["critical_Ved"] = Ved
        state["critical_theta"] = LMS_critical_theta_info["theta"]
        state["critical_pl_ratio"] = LMS_critical_theta_info["pl_ratio"]
        state["critical_n"] = LMS_critical_theta_info["n"]
        state["critical_sector"] = LMS_sector

    # --- Higher moment side ---
    (HMS_M_Rd_tot, HMS_M_Ed, _, _, HMS_plasticity_level_max_info,
     state["critical_theta_completed"], HMS_pl_ratio_values) = _perform_enhanced_rsm(
    ...
    if HMS_plasticity_level_max_info and not state["critical_pl_level_HMS_captured"]:
        ...
        state["theta_max_HMS"] = HMS_plasticity_level_max_info["theta"]
    ...
    if state["critical_theta_found"]:
        ...
        if converged_RE:
            optimal_DM_T = DM_T[THETAS, HMS_n_idxs_r_limit]
            max_theta = int(np.argmax(HMS_pl_ratio_r_converged))
            max_pl_ratio = HMS_pl_ratio_r_converged[max_theta]
            max_n_idx = HMS_n_idxs_r_limit[max_theta]
            max_DM_T = optimal_DM_T[max_theta]
            ...
            state["max_theta_HMS_RE"] = max_theta
...
def run_mode_plcap(h, h_o, b_f, t_w, t_f, r, f_y, M_V_Ratio, max_n=20, min_Ved=None):
    ...
    props, _, V_Rd, _ = build_section_properties(h, h_o, b_f, t_w, t_f, r, f_y)
    ...
    if min_Ved is None:
        min_Ved = run_mode_el1(h, h_o, b_f, t_w, t_f, r, f_y, M_V_Ratio).V_ed_EL1 * 1e3
    ...
    theta_LMS = UT_state["critical_theta"]
    if not theta_LMS:
        theta_LMS = int(np.argmax(np.abs(s_edge_360)))
    ...
    return PLCAPResult(
        ...
        theta_HMS=UT_state["theta_max_HMS"],
        pl_ratio_HMS=UT_state["pl_level_max_HMS"],
        n_HMS=UT_state["n_max_HMS"],
        theta_HMS_RE=UT_state["max_theta_HMS_RE"],
...
if __name__ == "__main__":
    ...
    result = run_mode_plcap(
        h=449.8, h_o=337.35, b_f=152.4, t_w=7.6, t_f=10.9, r=10.2,
        f_y=355, M_V_Ratio=1.333,
    )
    print("Mode PLCAP - Plastic Capacity")
    print(f"  duration            : {result.duration:.3f} s")
    print("  Without redistribution:")
    print(f"    V_ed = {result.V_ed_pl:.0f} kN   M_ed = {result.M_ed_pl:.0f} kNm")
    print(f"    LMS  theta={result.theta_LMS} deg  pl_ratio={result.pl_ratio_LMS:.1f}%  n={result.n_LMS:.2f}")
    if result.pl_ratio_HMS is not None:
        print(f"    HMS  theta={result.theta_HMS} deg  pl_ratio={result.pl_ratio_HMS:.1f}%  n={result.n_HMS:.2f}")
    ...
    if result.theta_HMS_RE is not None:
        print(f"    HMS  theta={result.theta_HMS_RE} deg  pl_ratio={result.pl_ratio_HMS_RE:.1f}%  n={result.n_HMS_RE:.2f}")
