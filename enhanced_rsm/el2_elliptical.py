...
condition through :attr:`EL2EllipticalResult.redistribution_required`; deciding what to do
about it is left to the caller.

The numerical kernels are imported from :mod:`enhanced_rsm.core_elliptical`; this module
...
class EL2EllipticalResult(NamedTuple):
    ...
    point_critical_Q2, point_critical_Q3 : int
    ...
    point_critical_Q1, point_critical_Q4 : int
    ...
    s_edge_max_Q1, s_edge_max_Q4 : float
        Peak edge stress on the LMS quadrants at that shear (MPa), clipped to
        +/- f_y where the section has yielded.
...
def _solve_lms_plasticity(sector, Ved, N_T, props, n_expanded, n_values):
    ...
    -------
    tuple
        ``(point_critical, pl_ratio, n, s_edge, s_edge_max)``. The edge stresses
        are clipped to +/- f_y, since the yielded fibres cannot carry more.
    ...
    # Beyond first yield the elastic expression N/A + Mc/I is a fictitious
    # extrapolation: with an elastic-perfectly-plastic material the extreme fibre
    # cannot carry more than f_y. Clipping reports the yielded plateau instead,
    # and keeps the peak consistent with the assembled 360-degree distribution.
    s_edge = np.clip(
        compute_elastic_stress(sector, N_th, M_th, props), -props.f_y, props.f_y
    )
    ...
    *_, r = calculate_moment_capacity(
    ...
    point_critical = int(np.argmax(pl_ratio_converged))
    return (
        point_critical,
        float(pl_ratio_converged[point_critical]),
        float(n_values[n_idxs[point_critical]]),
        s_edge,
        s_edge_max,
    )


def run_mode_el2_elliptical(h, h_o, a_b_ratio, b_f, t_w, t_f, r, f_y, M_V_Ratio,
                            max_n=20):
    ...
    V_ed_EL2_UT, point_Q2, s_edge_Q2 = solve_elastic_limit(2, V_ed_values, N_T_values, props)
    ...
    point_Q1, pl_ratio_Q1, n_Q1, s_edge_Q1, s_edge_max_Q1 = _solve_lms_plasticity(
        1, V_ed_EL2_UT, N_T_EL2_UT, props, n_expanded, n_values
    )

    # Bottom Tee — Q3 (HMS).
    V_ed_EL2_BT, point_Q3, s_edge_Q3 = solve_elastic_limit(3, V_ed_values, N_T_values, props)
    ...
    # Bottom Tee — Q4 (LMS).
    point_Q4, pl_ratio_Q4, n_Q4, s_edge_Q4, s_edge_max_Q4 = _solve_lms_plasticity(
...
if __name__ == "__main__":
    ...
    # axis ratio a/b = 1.5, M/V = 0.667).
    result = run_mode_el2_elliptical(
        h=449.8, h_o=337.35, a_b_ratio=1.5, b_f=152.4, t_w=7.6, t_f=10.9,
        r=10.2, f_y=355, M_V_Ratio=0.667,
    )
    print("Elastic Limit Mode - EL2 (HMS)")
    print(f"  duration        : {result.duration:.3f} s")
    print(f"  V_ed (EL2, HMS) : {result.V_ed_EL2:.0f} kN")
    print(f"  M_ed (EL2)      : {result.M_ed_EL2:.0f} kNm")
    print(f"  Q2  point={result.point_critical_Q2:>2d}  s_edge_max={result.s_edge_max_Q2:7.1f} MPa")
    print(f"  Q3  point={result.point_critical_Q3:>2d}  s_edge_max={result.s_edge_max_Q3:7.1f} MPa")
    print(f"  Q1  point={result.point_critical_Q1:>2d}  pl_ratio={result.pl_ratio_Q1:6.2f}%  n={result.n_Q1:.2f}  s_edge_max={result.s_edge_max_Q1:7.1f} MPa")
    print(f"  Q4  point={result.point_critical_Q4:>2d}  pl_ratio={result.pl_ratio_Q4:6.2f}%  n={result.n_Q4:.2f}  s_edge_max={result.s_edge_max_Q4:7.1f} MPa")
