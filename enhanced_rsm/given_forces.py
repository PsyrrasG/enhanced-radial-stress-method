...
obtained from :mod:`enhanced_rsm.el1` unless supplied explicitly.
"""
...
class GivenForcesResult(NamedTuple):
...
def _perform_enhanced_rsm(sector, N_Q, Ved, N_T, props, n_expanded, n_values):
    ...
    if converged:
        theta = int(np.argmax(pl_ratio_converged))
        info = {
            "sector": sector,
            "theta": theta,
            "pl_ratio": float(pl_ratio_converged[theta]),
            "n": float(n_values[n_idxs[theta]]),
        }
    else:
...
def _compute_tee_state(LMS_sector, HMS_sector, state, Ved, N_T, N_Q_map, props,
    ...
    if LMS_info:
        state["theta_LMS"] = LMS_info["theta"]
        state["pl_ratio_LMS"] = LMS_info["pl_ratio"]
        state["n_LMS"] = LMS_info["n"]

    if HMS_info:
        state["theta_HMS"] = HMS_info["theta"]
    ...
    if LMS_info and LMS_info["pl_ratio"] >= 100.0:
        ...
        if converged_RE:
            optimal_DM_T = DM_T[THETAS, HMS_n_idxs]
            max_theta = int(np.argmax(HMS_pl_ratio_converged))
            ...
            state["theta_HMS_RE"] = max_theta
            state["pl_ratio_HMS_RE"] = float(HMS_pl_ratio_converged[max_theta])
            state["n_HMS_RE"] = float(n_values[HMS_n_idxs[max_theta]])
...
def run_mode_given_forces(h, h_o, b_f, t_w, t_f, r, f_y, Ved, Med,
    ...
    props, _, _, _ = build_section_properties(h, h_o, b_f, t_w, t_f, r, f_y)

    if Ved_EL1 is None:
        Ved_EL1 = run_mode_el1(h, h_o, b_f, t_w, t_f, r, f_y, M_V_Ratio).V_ed_EL1
    ...
    if Ved < Ved_EL1:
        ...
        return GivenForcesResult(
            duration=duration,
            V_ed=Ved, M_ed=Med, V_ed_EL1=Ved_EL1,
            elastic=True,
            theta_LMS=int(np.argmax(np.abs(s_edge[1]))),
            theta_HMS=int(np.argmax(np.abs(s_edge[2]))),
            pl_ratio_LMS=None, n_LMS=None,
            pl_ratio_HMS=None, n_HMS=None,
            redistribution=False, DM_T=None,
            theta_HMS_RE=None, pl_ratio_HMS_RE=None, n_HMS_RE=None,
    ...
    theta_LMS = UT_state["theta_LMS"]
    if theta_LMS is None:
        theta_LMS = int(np.argmax(np.abs(s_edge[1])))
    theta_HMS = UT_state["theta_HMS"]
    if theta_HMS is None:
        theta_HMS = int(np.argmax(np.abs(s_edge[2])))
    ...
    return GivenForcesResult(
        ...
        theta_HMS_RE=UT_state["theta_HMS_RE"],
...
if __name__ == "__main__":
    # Demonstration runs (UB 457x152x52, S355, 75%h circular opening).
    ...
    for Ved, Med, label in [(60, 80, "below the elastic limit"),
        ...
        if result.elastic:
            print(f"  LMS theta={result.theta_LMS} deg   HMS theta={result.theta_HMS} deg")
        else:
            print(f"  LMS  theta={result.theta_LMS} deg  pl_ratio={result.pl_ratio_LMS:.2f}%  n={result.n_LMS:.2f}")
            if result.pl_ratio_HMS is not None:
                print(f"  HMS  theta={result.theta_HMS} deg  pl_ratio={result.pl_ratio_HMS:.2f}%  n={result.n_HMS:.2f}")
            else:
                print(f"  HMS  theta={result.theta_HMS} deg  (still elastic)")
            print(f"  redistribution: {result.redistribution}")
            if result.redistribution:
                print(f"    DM_T = {result.DM_T:.2f} kNm")
                print(f"    HMS after: theta={result.theta_HMS_RE} deg  "
